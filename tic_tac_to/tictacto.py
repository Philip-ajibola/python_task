...
class TicTacTo:
    ...
    def check_if_player_win_by_row(self) -> str:
        for number in range(len(self.board[0])):
            if self.board[number][0] == self.board[number][1] and self.board[number][2] and self.board[number][0] != Value.EMPTY:
                return self.check_winner_with_players_values(self.board[number][0])

    def check_if_player_win_by_column(self) -> str:
        for number in range(len(self.board[0])):
            if self.board[0][number] == self.board[1][number] and self.board[2][number] and self.board[0][number] != Value.EMPTY:
                return self.check_winner_with_players_values(self.board[number][0])
    ...
    def check_if_player_win_diagonally(self) -> str:
        if self.board[0][0] == self.board[1][1] and self.board[2][2] and self.board[0][0] != Value.EMPTY:
